...
Reset_Handler   PROC
             ...
             ; Load the address of CPACR (Coprocessor Access Control Register)
                LDR.W R0, =0xE000ED88      ; CPACR address
  
; Read the current value of CPACR
                LDR   R1, [R0]
  
; Set bits 20-23 to 1 (enable full access to CP10 and CP11, i.e., the FPU)
                ORR   R1, R1, #(0xF << 20)
 
; Write the modified value back to CPACR
                STR   R1, [R0]

; Ensure the store is complete
                DSB

; Reset the pipeline and ensure FPU access takes effect immediately
                ISB
